...
<img src='../files/u_logo.jpg'  class="img-responsive" alt="Responsive image"/>
<div class="wrapper">
		...
		<table class="table">
             ...
             if ($result->num_rows > 0) {
              ...
              while($row = $result->fetch_assoc()) {
				  ...
				  $date= $row["date"];
				  $status= $row["status"];
				   $sqls = "SELECT * FROM user_info where user_id = '$student_no' ";
             $result = $conn->query($sqls);
			   while($row = $result->fetch_assoc()) {
				   $department_name= $row["department_name"];
			echo "<tr><td>" .$result_id. "</td><td>" . $student_no. "</td><td>". $student_name. "</td><td>" . $department_name. "</td><td>" .$score. "</td><td>" . $date."</td><td>". $status. "</td>";
