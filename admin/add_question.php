...
if(isset($_POST['addq'])) {
	include '../db_config/connection.php';
		...
		$check= "SELECT * FROM exam where question='$question'";
		$result = $conn->query($check);
		if ($result->num_rows > 0){
			echo "<script language='javascript'> alert('Question  Already Exit');</script>";
		}
		else{
			
	$sql= "INSERT INTO exam  VALUES ('$department_id', '$courseId', '', '$question', '$opt1', '$opt2', '$opt3', '$opt4', '$answer')";

if ($conn->query($sql)){ 
echo "<script language='javascript'> alert('Successfuly added');</script>";
}
else{
	echo "<script language='javascript'> alert('Fail to add');</script>";
}
  ...
  <aside class="main-sidebar">
		...
		<li class="treeview">
          ...
          <ul class="treeview-menu">
            <li><a href="add_department.php"><i class="fa fa-circle-o"></i> Add Department</a></li>
             <li><a href="delete_department.php"><i class="fa fa-circle-o"></i> Delete Department</a></li>
          </ul>
        </li>
        <li>
          <a href="#">
   ...
   <ul class="treeview-menu">
            <li><a href="results.php"><i class="fa fa-circle-o"></i> Results</a></li>
			 <li  class="active"><a href="add_question.php"><i class="fa fa-circle-o"></i> Add Questions</a></li>
...
?>
         ...
         <form  method="POST">
				...
				<select  class="form-control" rows="3" name="department_id"  required> 
	<?php
				 include '../db_config/connection.php';
$rs=("Select * from department order by  department_id");
...
?>
				...
				<label>Course</label>
				<select  class="form-control" rows="3" name="courseId"  required> 
	<?php
				 include '../db_config/connection.php';
$rs=("Select * from courses order by  courseId");
$result = $conn->query($rs);
...
?>
            ...
            <div class="box-footer clearfix">
              <button type="submit" class="pull-right btn btn-default" name="addq" id="sendEmail">Add Question
